...
void TransitionManager::_bind_methods() {
    ...
    ClassDB::bind_method(D_METHOD("queue_switch_at_bar", "bar_number"), &TransitionManager::queue_switch_at_bar);
...
void TransitionManager::queue_switch_at_bar(int bar_number) {
    queued_bar = bar_number;
...
void TransitionManager::_connect_clock() {
    ...
    const StringName sig_bar("bar");
    const StringName sig_marker("marker_passed");

    Callable on_bar = callable_mp(this, &TransitionManager::_on_clock_bar);
    Callable on_marker = callable_mp(this, &TransitionManager::_on_clock_marker_passed);

    if (clock->has_signal(sig_bar) && !clock->is_connected(sig_bar, on_bar)) {
        clock->connect(sig_bar, on_bar);
    }
...
void TransitionManager::_disconnect_clock() {
    ...
    if (old_clock) {
        ...
        if (old_clock->is_connected(sig_bar, on_bar)) {
            old_clock->disconnect(sig_bar, on_bar);
        }
...
void TransitionManager::_on_clock_bar(int bar_number) {
    if (queued_bar >= 0 && bar_number >= queued_bar) {
        UtilityFunctions::print("Transition triggered at bar:", bar_number);
        emit_signal("transition_triggered", bar_number, StringName());
        cancel_switch();
    }
}
